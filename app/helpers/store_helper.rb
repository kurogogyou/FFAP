module StoreHelper
	...
	def search_helper(qstring, brand_id, model_id, year)
		product_list = Product.all
		if !is_empty(brand_id)
		   product_list = product_list.where(:brand_id => brand_id)
  		end
  		if !is_empty(model_id) and !is_empty(year)

  		  true_model = get_true_model(VehicleModel.find(model_id).model_name, year)
